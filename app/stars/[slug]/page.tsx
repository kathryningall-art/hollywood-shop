...
import { getStarBySlug, getLooksByStarId } from "@/lib/data";
...
export default async function StarPage({ params }: Props) {
  ...
  const looks = getLooksByStarId(star.id);
      ...
      <section className="bg-navy py-12 mt-4">
        ...
        <div className="flex gap-4 px-6 mb-3 overflow-hidden">
          {Array.from({ length: 40 }).map((_, i) => (
            <div
              key={i}
              className="shrink-0 w-6 h-4 rounded-sm bg-cream/10"
            />
          ))}
        </div>
